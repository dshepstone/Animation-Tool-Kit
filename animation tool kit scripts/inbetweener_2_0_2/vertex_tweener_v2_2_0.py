...
def _world_tween_targets():
    """Return the objects a World Tweener drag would operate on.

    Mirrors the engine's own resolution order: objects owning selected
    Graph Editor keys first, then the viewport transform selection.
    """
    objs = WorldTweenEngine.get_selected_keyframe_objects()
    if not objs:
        objs = cmds.ls(selection=True, type=('transform', 'joint')) or []
    return objs


def _show_world_tween_limit_popup(num_selected):
    """Modal reminder that the World Tweener is single-controller only."""
    try:
        cmds.confirmDialog(
            title='World Tweener',
            message=(
                'The World Tweener works on ONE controller at a time.\n\n'
                '{} objects are currently selected.\n\n'
                'Select a single controller or object, then use the '
                'slider again.'
            ).format(num_selected),
            button=['OK'],
            defaultButton='OK',
        ...
        cmds.warning('World Tweener works on one controller at a time.')
...
class SliderSession(object):
    ...
    def __init__(self, slider_type, chunk_name=None):
        ...
        self.active = False
        self.block_message = None  # set when begin() refuses the selection
    ...
    def begin(self):
        ...
        try:
            ...
            if st == 'LT':
                ...
                # The World Tweener is single-controller only: world-space
                # matrix tweening of several objects at once (especially
                # parent + child) produces unpredictable results.
                targets = _world_tween_targets()
                if len(targets) > 1:
                    self.block_message = (
                        "World Tweener: {} objects selected — select ONE "
                        "controller at a time".format(len(targets)))
                    # Close the undo chunk BEFORE the modal popup so the
                    # chunk can never leak if Qt drops the release event.
                    self._cleanup()
                    _show_world_tween_limit_popup(len(targets))
                    return 0
...
class VertexTickedSlider(QtWidgets.QSlider):
    ...
    larger square end caps, and a dark rounded box at the handle showing the
    mode label (LT/WT/BN/BD/BE) in the mode's accent color.
    """

    # Accent color per mode label (matches SliderPopOut.CONFIGS colors)
    _ACCENTS = {
        'LT': '#6BB5FF',
        'WT': '#FFD700',
        'BN': '#7EC8A0',
    ...
    def __init__(self, orientation, is_tw=True, is_world=False, label_text="", parent=None):
        ...
        self.label_text = label_text  # Text to display in center box (e.g., "WT", "BN")
    ...
    def _accent_color(self):
        hex_color = self._ACCENTS.get(self.label_text)
    ...
    def mousePressEvent(self, event):
        ...
        # cache, valueChanged -> tween, sliderReleased -> key) instead of
        # Qt's default page-step jump, which changes values WITHOUT ever
        # firing sliderPressed/sliderReleased — i.e. outside any undo
        # chunk and without keying the result.
    ...
    def paintEvent(self, event):
        ...
        for val in range(s_min, s_max + 1, 10):
            x = groove.left() + inset + ((val - s_min) / s_range) * span
            is_end = val in (s_min, s_max)
            size = 8.0 if is_end else (5.0 if val % 50 == 0 else 4.0)

            if self.is_tw and not self.is_world and (val < 0 or val > 100):
                color = QtGui.QColor(255, 130, 80)   # LT overshoot zone
            elif is_end:
                color = QtGui.QColor(accent)
            else:
                color = QtGui.QColor(dim)
            painter.setBrush(color)
            painter.drawRoundedRect(
                QtCore.QRectF(x - size / 2.0, cy - size / 2.0, size, size),
                1.5, 1.5)

        # --- Keyed-position tick (bright green diamond) ---
        if self.keyed_value is not None:
            kx = groove.left() + inset + ((self.keyed_value - s_min) / s_range) * span
            keyed_color = QtGui.QColor(0, 230, 120)
            painter.setBrush(keyed_color)
            painter.setPen(QtGui.QPen(QtGui.QColor(0, 180, 90), 1))
            diamond = QtGui.QPolygonF([
                QtCore.QPointF(kx, cy - 7),
                QtCore.QPointF(kx + 5, cy),
                QtCore.QPointF(kx, cy + 7),
                QtCore.QPointF(kx - 5, cy),
            ])
    ...
    def _on_pressed(self):
        ...
        if not count:
            self.status_label.setText(
                self.session.block_message
                or _no_target_message(self.slider_type))
...
class VertexTweenerUI(QtWidgets.QDialog):
    ...
    def create_layout(self):
        ...
        wt_layout.addWidget(self._make_descriptor(
            ...
            "Works on ONE controller or object at a time — select a single "
            "control before dragging."
        ))
    ...
    def _begin_session(self, slider_type):
        ...
        if not count:
            self.status_label.setText(
                session.block_message or _no_target_message(slider_type))
        return session
    ...
    def show_help_dialog(self):
        ...
        <div class="section">
            ...
            <ul>
                ...
                <li><span class="key">One at a time</span>: works on a SINGLE controller or
                object — selecting more than one shows a reminder popup</li>
            </ul>
