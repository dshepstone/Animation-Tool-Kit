...
class TestVisibilityRole:
    def test_visible_role_defaults_true(self):
        model = BookmarkListModel()
        model.refresh([make_bookmark()])
        assert model.data(model.index(0), BookmarkListModel.VisibleRole) is True

    def test_visible_role_reflects_bookmark_flag(self):
        model = BookmarkListModel()
        model.refresh([make_bookmark(visible=False)])
        assert model.data(model.index(0), BookmarkListModel.VisibleRole) is False

    def test_eye_rect_is_within_row_rect(self):
        from time_bookmarks.qt_compat import QtCore
        row = QtCore.QRect(0, 0, 400, 40)
        eye = BookmarkDelegate.eye_rect(row)
        assert row.contains(eye)

    def test_eye_click_emits_toggle_signal(self, qtbot):
        ...
        w.show()
        qtbot.waitExposed(w)
        b = make_bookmark(name="Eye Test")
        w.set_bookmarks([b])
        # Allow the view to lay out rows before querying visualRect.
        QtWidgets.QApplication.processEvents()

        received = []
        w.bookmark_visibility_toggled.connect(received.append)

        index = w._model.index(0)
        row_rect = w._list_view.visualRect(index)
        if row_rect.isEmpty():
            # Fall back to a synthetic row rect if the offscreen view has
            # not laid out yet — still exercises the hit-test path.
            row_rect = QtCore.QRect(0, 0, w._list_view.viewport().width() or 300, 40)
        eye_rect = BookmarkDelegate.eye_rect(row_rect)
        pos = eye_rect.center()

        try:
            btn = QtCore.Qt.MouseButton.LeftButton
            evt_type = QtCore.QEvent.Type.MouseButtonPress
            no_mod = QtCore.Qt.KeyboardModifier.NoModifier
        except AttributeError:
            btn = QtCore.Qt.LeftButton
            evt_type = QtCore.QEvent.MouseButtonPress
            no_mod = QtCore.Qt.NoModifier
        ...
        consumed = w.eventFilter(w._list_view.viewport(), event)
